...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['patient_id']) && (string) $_GET['patient_id'] !== '') {
    ...
    if ($pid === false) {
        ...
        $p = db()->prepare(
            'SELECT Patient_ID AS patient_id, Full_Name AS full_name, Age AS age,
                    Gender AS gender, Ins_Type AS ins_type, Provider AS provider,
        ...
        if (!$searchPatient) {
            ...
            if ($searchError === null) {
                // Procedure_Name is the column name in the schema
                $sql = 'SELECT v.Visit_ID AS visit_id, v.Patient_ID AS patient_id,
                               v.Doctor_ID AS doctor_id, v.Procedure_Name AS procedure_name,
                               v.Cost AS cost, v.Length_of_Stay AS length_of_stay,
                               v.Satisfaction AS satisfaction, v.Outcome AS outcome,
                               v.Re_Admission AS re_admission,
                ...
                $sql .= ' GROUP BY v.Visit_ID, v.Patient_ID, v.Doctor_ID, v.Procedure_Name,
                                   v.Cost, v.Length_of_Stay, v.Satisfaction, v.Outcome, v.Re_Admission
...
<body>
  <main>
    ...
    <?php elseif ($searchPatient !== null): ?>
      ...
      <h2 style="font-size:1.05rem;margin-top:1.5rem">Visits<?= (!empty($_GET['condition_id'])) ? ' (filtered)' : '' ?></h2>
      <?php if ($searchVisits === []): ?>
